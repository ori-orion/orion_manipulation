...
HandleCropper::HandleCropper(const ros::Publisher& cloud_pub,
                             const ros::Publisher& door_pub)
    : cloud_pub_(cloud_pub), door_pub_(door_pub) {}

HandleCentroid::HandleCentroid(const tf::TransformBroadcaster& br)
    : handle_tf_br_(br), good_detection_(false) {}

DrawerHandleCropper::DrawerHandleCropper(const ros::Publisher& cloud_pub,
                                         const ros::Publisher& plane_pub)
    : cloud_pub_(cloud_pub), plane_pub_(plane_pub) {}

DrawerHandleCentroid::DrawerHandleCentroid(const tf::TransformBroadcaster& br)
    : handle_tf_br_(br), good_detection_(false) {}
...
void HandleCentroid::Callback(const sensor_msgs::PointCloud2& msg) {
  ...
  handle_tf_br_.sendTransform(tf::StampedTransform(
      transform, ros::Time::now(), "head_rgbd_sensor_rgb_frame", "door_handle"));
...
bool HandleCentroid::CheckDetection() { return HandleCentroid::good_detection_; }

double HandleCentroid::GetX() { return HandleCentroid::x_; }

double HandleCentroid::GetY() { return HandleCentroid::y_; }

double HandleCentroid::GetZ() { return HandleCentroid::z_; }
...
void DrawerHandleCropper::Callback(const sensor_msgs::PointCloud2& msg) {
  ...
  PointCloudC::Ptr first_cropped_cloud(new PointCloudC());
  CropCloud(cloud, first_cropped_cloud, min_crop_pt, max_crop_pt);
  ROS_INFO("Initial crop leaves point cloud with %ld points",
  ...
  //------ Crop the point cloud used to get the handle --------
  Eigen::Vector4f min_pt(min_x + 0.1, min_y + 0.03, min_z, 1);
  ...
  if (clusters.size() > 0) {
    for (size_t i = 0; i < clusters.size(); ++i) {
      ...
      *handle_inliers = clusters.at(i);
      ...
      //            std::string drawer_handle_name = "drawer_handle_" + ss.str();
      handle_tf_br_.sendTransform(tf::StampedTransform(
          transform, ros::Time::now(), "head_rgbd_sensor_rgb_frame", drawer_handle_name));
...
std::vector<double> DrawerHandleCentroid::GetX() { return DrawerHandleCentroid::x_; }

std::vector<double> DrawerHandleCentroid::GetY() { return DrawerHandleCentroid::y_; }

std::vector<double> DrawerHandleCentroid::GetZ() { return DrawerHandleCentroid::z_; }
