...
bool detect_bin_handle(point_cloud_filtering::DetectBinHandle::Request &req,
  ...
  // Crop for the handle
  point_cloud_filtering::BinHandleCropper bin_handle_cropper(bin_handle_pub,
                                                             bin_surface_pub);
  point_cloud_filtering::BinHandleCentroid bin_handle_centroid(br);

  ros::Subscriber sub_handle =
      nh.subscribe("cloud_in", 1, &point_cloud_filtering::BinHandleCropper::Callback,
                   &bin_handle_cropper);

  ros::Subscriber sub_handle_centroid = nh.subscribe(
      "bin_handle_cloud", 1, &point_cloud_filtering::BinHandleCentroid::Callback,
      &bin_handle_centroid);
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer service =
      nh.advertiseService("bin_handle_detection", detect_bin_handle);
