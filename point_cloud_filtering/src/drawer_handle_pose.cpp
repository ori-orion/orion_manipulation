...
bool detect_handle(point_cloud_filtering::DetectDrawerHandles::Request  &req,
    ...
    ros::Subscriber joint_sub =
            nh.subscribe ("/hsrb/robot_state/joint_states", 1, &point_cloud_filtering::DrawerHandleCropper::GetHeadAngle, &handle_cropper);

    ros::Subscriber sub_handle =
            nh.subscribe("cloud_in", 1, &point_cloud_filtering::DrawerHandleCropper::Callback,  &handle_cropper);

    ros::Subscriber sub_handle_centroid =
            nh.subscribe("handle_cloud", 1, &point_cloud_filtering::DrawerHandleCentroid::Callback,  &handle_centroid);
...
int main(int argc, char** argv) {
    ...
    ros::ServiceServer service = nh.advertiseService("drawer_handle_detection", detect_handle);
