...
BinHandleCropper::BinHandleCropper(const ros::Publisher& bin_handle_pub,
                                   const ros::Publisher& bin_surface_pub)
    : bin_handle_pub_(bin_handle_pub), bin_surface_pub_(bin_surface_pub) {}

BinHandleCentroid::BinHandleCentroid(const tf::TransformBroadcaster& br)
    : bin_handle_tf_br_(br), good_detection_(false) {}
...
void BinHandleCentroid::Callback(const sensor_msgs::PointCloud2& msg) {
  ...
  bin_handle_tf_br_.sendTransform(tf::StampedTransform(
      transform, ros::Time::now(), "head_rgbd_sensor_rgb_frame", "bin_handle"));
...
bool BinHandleCentroid::CheckDetection() { return BinHandleCentroid::good_detection_; }

double BinHandleCentroid::GetX() { return BinHandleCentroid::x_; }

double BinHandleCentroid::GetY() { return BinHandleCentroid::y_; }

double BinHandleCentroid::GetZ() { return BinHandleCentroid::z_; }
