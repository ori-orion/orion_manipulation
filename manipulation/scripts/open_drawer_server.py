...
class OpenDrawerAction(object):
    ...
    def get_handle_poses(self):
        ...
        try:
            detect_handle_service = rospy.ServiceProxy(
                "/drawer_handle_detection", DetectDrawerHandles
            )
    ...
    def get_goal_pose(self, relative=geometry.pose(), offset=0.0):
        ...
        while not foundTrans:
            try:
                odom_to_ref_pose = self.whole_body._lookup_odom_to_ref(
    ...
    def execute_cb(self, goal_msg):
        ...
        if num_handles_found > 0:
            self.tts.say(
                "I found %d handles and will now grasp one of them" % num_handles_found
            )

            # Grasp the handle
            rospy.loginfo("%s: Grasping handle..." % (self._action_name))
            rospy.sleep(1)
            handle_goal_pose = self.get_goal_pose(
                relative=geometry.pose(
                    x=handle_poses.x[0], y=handle_poses.y[0], z=handle_poses.z[0]
                ),
                offset=-0.15,
            )
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(z=0.055), "hand_palm_link"
            )
            ...
            try:
                rospy.loginfo(
                    "%s: Attempting to move backwards..." % (self._action_name)
                )
                ...
                rospy.loginfo(
                    "%s: Attempting to move back more..." % (self._action_name)
                )
                self.whole_body.move_end_effector_pose(
                    geometry.pose(z=-0.1), "hand_palm_link"
                )
