    ...
    def execute_cb(self, goal_msg):
        ...
        while goal_tf is None:
            ...
            if goal_tf is not None:
                rospy.loginfo(
                    '{0}: Choosing tf frame "{1}".'.format(
                        self._action_name, str(goal_tf)
                    )
                )
        ...
        while True:
            ...
            while not can_look:
                try:
                    ...
                    if self._as.is_preempt_requested():
                        rospy.loginfo(
                            "%s: Preempted. Moving to go and exiting."
                            % self._action_name
                        )
                        self.whole_body.move_to_go()
                        self._as.set_preempted()
            ...
            distance = math.sqrt(
                math.pow(person_coords[1], 2) + math.pow(person_coords[0], 2)
            )
            theta = math.atan(person_coords[1] / person_coords[0])
            rospy.loginfo(
                '{0}: Distance to person: "{1} Theta: {2}".'.format(
                    self._action_name, str(distance), str(theta)
                )
            )
            ...
            if distance <= 0.5:
                ...
                base_pose = geometry.pose(
                    ratio * person_coords[0],
                    ratio * person_coords[1],
                    0.0,
                    0.0,
                    0.0,
                    theta,
