...
class PutObjectOnFloorAction(object):
    ...
    def execute_cb(self, goal_msg):
        ...
        try:
            rospy.loginfo(
                "%s: Placing gripper close to floor in front" % (self._action_name)
            )
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(x=-0.2, y=0, z=0.3), "hand_palm_link"
            )
            ...
            rospy.loginfo(
                "{0}: The weight change by grams is {1}.".format(
            ...
            if weight > 100:
                self.tts.say(
                    "{0}: I can still feel the object.".format(self._action_name)
                )
                rospy.sleep(3)
