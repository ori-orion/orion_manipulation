...
class OpenBinLidAction(object):
    ...
    def get_bin_handle_poses(self):
        ...
        try:
            detect_handle_service = rospy.ServiceProxy(
                "/bin_handle_detection", msg.DetectDrawerHandles
            )
    ...
    def get_head_frame_object_pose(self, object_tf):
        ...
        while not found_trans:
            try:
                ...
                (trans, rot) = listen.lookupTransform(
                    "/head_rgbd_sensor_rgb_frame", object_tf, t
                )
                found_trans = True
            except (
                tf.LookupException,
                ...
                if self._as.is_preempt_requested():
                    rospy.loginfo(
                        "%s: Preempted. Moving to go and exiting." % self._action_name
                    )
    ...
    def get_goal_pose(self, relative=geometry.pose()):
        ...
        eulers = T.euler_from_quaternion(
            [
    ...
    def collision_mod(self, msg):
        if self.goal_object:
            ...
            for i in range(len(message.poses)):
                ...
                if not (
                    np.all(pose_arr <= upper_bound) and np.all(pose_arr >= lower_bound)
                ):
                    inds_to_remove.append(i)

                # approx remove the object
                if np.all(pose_arr <= object_upper_bound) and np.all(
                    object_lower_bound >= lower_bound
                ):
                    inds_to_remove.append(i)
    ...
    def execute_cb(self, goal_msg):
        ...
        rospy.Subscriber(
            "known_object_pre_filter", msg.CollisionObject, self.collision_callback
        )
        ...
        # Set collision map
        self.tts.say(
            "I am now evaluating my environment so that I don't collide with anything."
        )
        rospy.sleep(1)
