...
def analyse_hand_image(hand_cam_topic, height_above_object):
    ...
    rgb_sub = message_filters.Subscriber(
        hand_cam_topic, Image, queue_size=1, buff_size=2**24
    )
    ...
    mask = np.logical_and(
        np.logical_and(image_cp[:, :, 0] < 255, image_cp[:, :, 1] < 255),
        image_cp[:, :, 2] < 255,
    )
    ...
    theta = np.mean(
        np.arctan(
            np.divide(
                -y_coords_no_mean_pos.astype(float), x_coords_no_mean_pos.astype(float)
            )
        )
        * 180
        / math.pi
...
class ForceSensorCapture(object):
    ...
    def __init__(self):
        ...
        # Wait for connection
        try:
            rospy.wait_for_message(ft_sensor_topic, WrenchStamped, timeout=10)
...
class PickUpBinBagAction(object):
    def __init__(self, name):
        ...
        self.whole_body.planning_timeout = (
            20.0  # Increase planning timeout. Default is 10s
        )
    ...
    def pick_up_bin_lid(self):
        ...
        try:
            ...
            try:
                ...
                # Move gripper above bin
                rospy.loginfo(
                    "%s: Encountered an error. Trying again." % self._action_name
                )
                self.move_above_bin()
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(z=0.30), "hand_palm_link"
            )
            ...
            for i in range(3):
                x_to_move, y_to_move, theta = analyse_hand_image(
                    self.hand_cam_topic, height_above_object
                )
                theta_rad = theta * math.pi / 180.0
                rospy.loginfo(
                    '{:}: Need to rotate "{:.2f}" degrees.'.format(
                        self._action_name, theta
                    )
                )
                self.tts.say('I will rotate "{:.2f}" degrees.'.format(theta))

                self.whole_body.move_end_effector_pose(
                    geometry.pose(x=x_to_move), "hand_palm_link"
                )
                rospy.sleep(1)
                self.whole_body.move_end_effector_pose(
                    geometry.pose(y=y_to_move), "hand_palm_link"
                )
                rospy.sleep(1)
                self.whole_body.move_end_effector_pose(
                    geometry.pose(ek=-theta_rad), "hand_palm_link"
                )
                rospy.sleep(1)

            self.whole_body.move_end_effector_pose(
                geometry.pose(z=height_above_object - 0.08), "hand_palm_link"
            )
            ...
            force_difference = compute_difference(
                pre_grasp_force_list, post_grasp__force_list
            )
            weight = math.floor(force_difference / 9.81 * 1000)

            print("The weight is " + str(weight) + "grams.")
            rospy.loginfo(
                "{0}: The weight in grams is {1}.".format(
                    self._action_name, str(weight)
                )
            )
            self.tts.say(
                "{0}: I can feel a weight of {1} grams.".format(
                    self._action_name, str(weight)
                )
            )
            rospy.sleep(3)
            ...
            if weight > 300:
                ...
                if self.counter > 0:
                    self.whole_body.move_end_effector_pose(
                        geometry.pose(z=-0.1), "hand_palm_link"
                    )
                ...
                self.tts.say(
                    "Encountered a problem. Please could you remove the bin lid for me."
                )
                rospy.sleep(10)
    ...
    def move_above_bin(self):
        ...
        self.whole_body.move_end_effector_pose(
            geometry.pose(x=0.4, z=1.0, ei=math.pi), "base_footprint"
        )

    def execute_cb(self, goal_msg):
        # Messages for feedback / results
        _result = msg.PickUpBinBagResult()
        ...
        try:
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(z=0.10), "hand_palm_link"
            )
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(z=-0.60), "hand_palm_link"
            )
            ...
            # Return to "go" pose
            rospy.loginfo(
                "%s: Moving bin higher to avoid the laser." % (self._action_name)
            )
            self.tts.say("Moving bin higher to avoid the laser.")
            rospy.sleep(1)
            self.whole_body.move_end_effector_pose(
                geometry.pose(x=0.5), "hand_palm_link"
            )
