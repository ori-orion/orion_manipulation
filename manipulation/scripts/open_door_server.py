...
class OpenDoorAction(object):
    ...
    def execute_cb(self, goal_msg):
        rospy.loginfo(
            "%s: Executing callback. Moving into position" % (self._action_name)
        )
        ...
        if not handle_pose.handle_detected:
            rospy.loginfo(
                "%s: Could not find door handle. Handle detection timed out."
                % self._action_name
            )
        ...
        self.whole_body.move_end_effector_pose(
            geometry.pose(x=handle_pose.x, y=handle_pose.y, z=handle_pose.z - 0.08),
        ...
        if handle_pose.x > 0:
            ...
            self.whole_body.move_end_effector_pose(
                geometry.pose(x=-0.06), "hand_palm_link"
            )
        else:
        ...
        try:
            self.whole_body.move_end_effector_pose(
                geometry.pose(z=0.05), "hand_palm_link"
            )
        except:
        ...
        try:
            self.whole_body.move_end_effector_pose(
                geometry.pose(y=0.06), "hand_palm_link"
            )
            rospy.loginfo(
                "%s: Successfully pulled handle down..." % (self._action_name)
            )
        except:
        ...
        self.whole_body.move_to_go()
        rospy.loginfo(
            "%s: Succeeded door opening. Now returning results." % self._action_name
        )
