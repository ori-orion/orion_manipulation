...
class PickUpObjectAction(ManipulationAction):
    ...
    GOAL_OBJECT_TF_TIMEOUT = 3.0  # How recently we must have seen an object to pick it up
    ...
    def __init__(self, action_name, action_msg_type=orion_actions.msg.PickUpObjectAction, use_collision_map=True, use_grasp_synthesis=True):

        super(GraspingManipulationAction, self).__init(action_name, action_msg_type, use_collision_map)
        ...
        if self.use_grasp_synthesis:
            ...
            rospy.loginfo("%s: Waiting for object_segmentation service..." % self._action_name)
            rospy.wait_for_service('/object_segmentation')
            self.segment_object_service = rospy.ServiceProxy('/object_segmentation', SegmentObject)
    ...
    def _execute_cb(self, goal_msg):
        ...
        rospy.loginfo("%s: Distance to object is %s m from hand."
                      % (self._action_name, obj_dist))
        ...
        if self.use_collision_map:
            self.tts_say("I am now evaluating my environment so I don't collide with anything.")
            ...
            self.collision_world = self.collision_mapper.build_collision_world(
                ...
                crop_bounding_boxes=[object_bounding_box]
            )
    ...
    def suck_object(self, goal_tf):
        ...
        # Wait for connection
        try:
            if not suction_control_client.wait_for_server(rospy.Duration(self.ACTION_SERVER_CONNECTION_TIMEOUT)):
